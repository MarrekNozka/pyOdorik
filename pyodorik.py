...
'''.format(self.conffile))
            sys.exit(10)

        self.config = configparser.ConfigParser()
        self.config.read(self.conffile)
        if 'auth' in self.config and \
                'user' in self.config['auth'] and \
                'password' in self.config['auth']:
...
'''.format(self.conffile))
            sys.exit(20)
        if 'default_caller' in self.config and \
                self.hostname in self.config['default_caller']:
            self.caller = self.config['default_caller'][self.hostname]
        elif 'default_caller' in self.config and \
                'default' in self.config['default_caller']:
...
'''.format(self.conffile, self.hostname))
            sys.exit(21)
...
""")
    ...
    def make(self, args):
        ...
        if not re.search('^\d+$', self.caller):
            self.caller = self.findContact(self.caller)
        if not re.search('^\d+$', self.recipient):
    ...
    def findContact(self, exp):
        ...
        def match(item):
            exppp = unicodedata.normalize('NFD', exp).upper()\
                  .encode('ascii', 'ignore').decode('ascii')
            return re.search(exppp,
        ...
        if len(calllist) == 0:      # nic jsem nenašel
            ...
            for i, contact in enumerate(calllist, start=1):
                print(' ', i, ':',
                      contact['shortcut'], contact['name'], contact['number'])
    ...
    def call(self):
        ...
        if self.caller and self.recipient:
            return self.getFromAPI("POST", '/callback',
                                   caller=self.caller,
